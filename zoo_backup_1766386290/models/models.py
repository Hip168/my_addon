...
class ZooFood(models.Model):
    _name = "zoo.food"
    _description = "Thức ăn"

    name = fields.Char(string="Tên thức ăn", required=True)
...
class Cage(models.Model):
    ...
    species_id = fields.Many2one(
        "animal.species", string="Dành riêng cho Loài", required=True
    )
...
class AnimalBase(models.Model):
    ...
    cage_id = fields.Many2one(
        "cage", string="Chuồng nuôi", domain="[('species_id', '=', species_id)]"
    )
...
class AnimalBaseExtension(models.Model):
    ...
    health_status = fields.Selection(
        [("good", "Tốt"), ("sick", "Đang bệnh"), ("recovering", "Đang hồi phục")],
...
class RaceHorse(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        horse_species = self.env["animal.species"].search(
            [("name", "=", "Ngựa")], limit=1
        )
        if horse_species:
